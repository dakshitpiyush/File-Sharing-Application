...
public class Sharing extends AppCompatActivity {
    ...
    private final ViewGroup scroll = findViewById(R.id.scrShare);
    ...
    private View curSendView, curReceiveView;
    private long curReceived = 0, curSent = 0, curRecFileSize = 0, curSentFileSize = 0;
    ...
    public void send(String url) {
        ...
        View fileShareView = layoutInflater.inflate(R.layout.file_trans, null);
        ...
        curSendView = fileShareView;
        curSent = 0;
        curSentFileSize = curFile.length();
    ...
    private void makeProgress(boolean isSend) {
        ...
        if (isSend) {
            curSent += SendReciveFile.BUFFER_SIZE;
            progress = Math.min(curSent, curSentFileSize);
            fileSharingView = curSendView;
        } else {
            curReceived += SendReciveFile.BUFFER_SIZE;
            fileSharingView = curReceiveView;
            progress = Math.min(curReceived, curRecFileSize);
        }
